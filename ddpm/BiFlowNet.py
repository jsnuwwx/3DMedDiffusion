...
class BiFlowNet(nn.Module):
    def __init__(
        ...
        self.init_conv = nn.Conv3d(channels * 2, init_dim, (init_kernel_size, init_kernel_size,
                                                            init_kernel_size),
    ...
    def forward(
        ...
        b = x.shape[0]
        # 🛠️ 修正：这里应该是 4，因为我们用的是 4x AE (24 -> 96)
        ori_shape = (x.shape[2]*4, x.shape[3]*4, x.shape[4]*4) 
        # time_rel_pos_bias = self.time_rel_pos_bias(x.shape[2], device=x.device)
        x_IntraPatch = x.clone()
        # x_IntraPatch.retain_grad() 
        x_IntraPatch = x_IntraPatch.unfold(2, self.sub_volume_size[0], self.sub_volume_size[0]) \
                                   .unfold(3, self.sub_volume_size[1], self.sub_volume_size[1]) \
                                   .unfold(4, self.sub_volume_size[2], self.sub_volume_size[2])
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, denoise_fn, x_start, t, y=None, res=None, noise=None, hint=None, **kwargs):
        ...
        if is_list_str(y):
            y = bert_embed(tokenize(y), return_cls_repr=self.text_use_bert_cls)
            y = y.to(device)

        # 🚨 核心修复：把原作者的 hint == None 改成 hint is None
        if hint is None:
